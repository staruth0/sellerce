	...
	return (
		<div className="slide-content-container">
			...
			<div className="showcase-image">
				<img className="hero-image" src={data.image[Varients.length && activeColorSrc]} alt="landing-img" />
				<div className={data.image.length ? "color-variants" : "hide"}>
					{Varients.map((url, index) => (
						<div
							>
							<img key={index} src={url} alt={`color ${index + 1}`} className={
								index === 0 ? "active color" : "color"
							}/>
						</div>
					))}
				</div>
